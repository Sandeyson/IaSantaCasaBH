...
def save_result_to_file():
    ...
    if st.session_state.test_end_time is None:
        st.session_state.test_end_time = datetime.now(TZ_BRASILIA)
    try:
...
def reset_test_data():
    st.session_state.update({
        ...
        "test_start_time": datetime.now(TZ_BRASILIA), "test_end_time": None, "result_saved": False,
...
def _render_scroll_anchor():
    ...
    ts = datetime.now(TZ_BRASILIA).timestamp()
    if should_scroll:

...
RAG_TOP_K = 4
APP_VERSION = "v8.2 - 27/05/2026 - RAG + OpenAI + MultipleChoice"
...
def init_state():
    defaults = {
        ...
        "index": 0, "results": [], "score": 0, "level": "", "chat": [],
...
def get_level_class(level):
    ln = (level or "").strip().lower()
    if ln in ("básico","basico"):
        return "level-basico"
    if ln in ("intermediário","intermediario"):
        return "level-intermediario"
    if ln in ("avançado","avancado"):
...
def save_result_to_file():
    ...
    try:
        ...
        sheet.append_row([
            ...
            st.session_state.name, st.session_state.matricula,
            st.session_state.unidade_negocio, st.session_state.level,
            f"{st.session_state.score}/5",
            st.session_state.extra_question_answer, st.session_state.extra_question_topic,
...
def reset_test_data():
    st.session_state.update({
        ...
        "level": "", "name": "", "matricula": "", "unidade_negocio": "",
...
def start_conversation():
    ...
    reset_test_data()
    add_msg("assistant", "Olá! Qual é o seu nome completo (nome e sobrenome)?", section="quiz")
...
def responder_pergunta_extra(resposta):
    ...
    if resposta == "Sim":
        ...
        add_msg("assistant", "Qual tema você deseja abordar no ensinamento?", section="quiz")
        st.session_state.scroll_to_bottom = True
        st.session_state.scroll_nonce = st.session_state.get("scroll_nonce", 0) + 1
    ...
    add_msg("assistant", "Resposta registrada no card do resultado final. A partir de agora você pode usar o chat para tirar dúvidas sobre inteligência artificial e sobre a Santa Casa BH.", section="quiz")
    st.session_state.focus_input = True
    st.session_state.scroll_to_bottom = True
    st.session_state.scroll_nonce = st.session_state.get("scroll_nonce", 0) + 1
    save_result_to_file()
...
def process_tema_compartilhamento(temas):
    ...
    st.session_state.scroll_nonce = st.session_state.get("scroll_nonce", 0) + 1
    st.session_state.test_end_time = datetime.now()
    add_msg("assistant", "Temas registrados com sucesso. A partir de agora você pode usar o chat para tirar dúvidas.", section="quiz")
    save_result_to_file()
    st.rerun()
...
# ═══════════════════════════════════════════════════════════════
#  _render_scroll_anchor — versão robusta Desktop + iOS + Android
# ═══════════════════════════════════════════════════════════════
def _render_scroll_anchor():
    """
    Scroll confiável para o final da página em todos os browsers.

    Melhorias vs versão anterior:
    - Timers escalonados: 50/200/500/900/1500ms cobrem iOS lazy layout
    - MutationObserver re-dispara após cada rerun do Streamlit
    - scrollIntoView no #fim-da-pagina (mais confiável no iOS Safari)
    - Todos os contêineres internos do Streamlit são scrollados
    - Nonce + timestamp impedem cache entre reruns
    """
...
<script>
/* scroll-anchor | nonce={nonce} | ts={ts} | go={go} */
(function() {{
    var GO = {go};
    ...
    /* Timers escalonados: cobre render rápido, lento e iOS */
    [50, 200, 500, 900, 1500].forEach(function(ms) {{
    ...
    try {{
        ...
        /* Para de observar após 4 segundos */
...
def run_scroll_to_bottom_script():
    pass  # substituído por _render_scroll_anchor()
    ...
    if keywords:
        ...
        if hits >= 1:
            return AnswerEvaluation(
                ...
                feedback="✅ Boa resposta! Você demonstrou compreensão do conceito."
            )
    llm = get_llm()
    keywords_str = ", ".join(keywords) if keywords else ""
    res = llm.invoke(f"""Você é um avaliador de conhecimento em IA para colaboradores da Santa Casa BH.
...
Resposta do usuário: {user_answer}""")
    ...
    if "feedback:" in content:
        feedback = res.content.split("feedback:", 1)[1].strip()
...
def process_name(user_text):
    ...
    if not nome_valido(user_text):
        add_msg("assistant",
            "Por favor, preencha seu **nome completo** (nome e sobrenome) para continuar. "
            "Não são aceitos apenas o primeiro nome, números, símbolos ou palavras inadequadas.",
            section="quiz")
    ...
    if not valido_openai:
        add_msg("assistant",
            f"Para prosseguir, informe seu **nome e sobrenome** completos. "
            f"Certifique-se de que o cadastro está correto e tente novamente.",
            section="quiz")
...
def process_matricula(user_text):
    ...
    if not matricula_valida(user_text):
        add_msg("assistant","Matrícula inválida. Informe apenas números, sem letras, espaços ou símbolos.",section="quiz")
        st.session_state.focus_input = True; st.session_state.scroll_to_bottom = True; st.session_state.scroll_nonce += 1; return
...
def process_unidade(unidade):
    ...
    st.session_state.scroll_to_bottom = True; st.session_state.scroll_nonce += 1
    add_msg("assistant","Deseja responder às perguntas sobre Inteligência Artificial para que seja validado o seu nível de conhecimento? Caso não queira participar, pode responder não. Seu nível será Básico.",section="quiz")
...
def process_multiple_choice(option_index):
    ...
    feedback_text = "✅ Correto!" if correct else f"❌ Incorreto. A resposta certa era: {current['options'][current['correct_option']]}"
    ...
    if st.session_state.index < len(st.session_state.questions):
        ...
        if next_q["type"] == "open":
            st.session_state.focus_input = True
            add_msg("assistant",f"Última pergunta — esta é aberta, responda com suas próprias palavras:\n\n{next_q['question']}",section="quiz")
        else:
            st.session_state.focus_input = False
            add_msg("assistant", next_q["question"], section="quiz")
        return
    _finalizar_quiz()
...
def _finalizar_quiz():
    ...
    st.session_state.scroll_to_bottom = True; st.session_state.scroll_nonce += 1
    add_msg("assistant",f"{st.session_state.name}, sua avaliação foi concluída. Confira abaixo seu resultado final e a revisão das perguntas.",section="quiz")
    if st.session_state.level in ["Intermediário","Avançado"]:
...
def render_header():
    ...
    with st.container():
        ...
        with col_texto:
            ...
            <div class="start-card-subtitle">Inicie sua avaliação de conhecimento em Inteligência Artificial.</div>
            </div>""", unsafe_allow_html=True)
        with col_botao:
...
def render_status():
    ...
    nivel     = st.session_state.level if st.session_state.quiz_completed else "-"
    st.markdown(f"""
    <div class="status-bar">
      ...
      <div class="status-item"><span class="status-label">Nível</span><span class="status-value">{nivel}</span></div>
    </div>
...
def render_multiple_choice_buttons():
    ...
    if current_q["type"] != "multiple_choice":
        return
    st.markdown(
        f'<div class="result-section-title">Pergunta {idx+1} de {len(st.session_state.questions)} — Selecione uma opção:</div>',
        unsafe_allow_html=True
    )
...
def render_confirmar_quiz_buttons():
    if not (st.session_state.started and st.session_state.phase == "confirmar_quiz"):
        return
    st.markdown('<div class="result-section-title">Deseja participar da avaliação?</div>', unsafe_allow_html=True)
    ...
    with col1:
        if st.button("✅ Sim", key="btn_confirmar_quiz_sim"):
            ...
            add_msg("assistant",f"Ótimo! Vamos começar. São 4 perguntas de múltipla escolha e 1 pergunta aberta.\n\n**Pergunta 1:** {first_q['question']}",section="quiz")
            st.rerun()
    with col2:
        if st.button("❌ Não", key="btn_confirmar_quiz_nao"):
            st.session_state.aceitou_responder_quiz = "Não"
            add_msg("user","Não",section="quiz")
            st.session_state.score = 0; st.session_state.level = "Básico"
            st.session_state.quiz_completed = True; st.session_state.extra_question_needed = False
            st.session_state.extra_question_answered = True; st.session_state.phase = "chat"
            st.session_state.focus_input = True; st.session_state.test_end_time = datetime.now()
            st.session_state.scroll_to_bottom = True; st.session_state.scroll_nonce += 1
            add_msg("assistant",f"{st.session_state.name}, sua participação foi registrada como não participante da avaliação. Seu nível foi definido como Básico.",section="quiz")
            save_result_to_file(); st.rerun()


def render_extra_question_buttons():
    if not (st.session_state.quiz_completed and st.session_state.phase == "mostrar_resultado"
            and st.session_state.extra_question_needed and not st.session_state.extra_question_answered):
        return
    st.markdown('<div class="result-section-title">Deseja compartilhar seus conhecimentos sobre Inteligência Artificial?</div>', unsafe_allow_html=True)
    col1, col2, *_ = st.columns([1, 1, 6])
    with col1:
        if st.button("✅ Sim", key="btn_extra_sim"): responder_pergunta_extra("Sim")
    with col2:
        if st.button("❌ Não", key="btn_extra_nao"): responder_pergunta_extra("Não")


def render_tema_compartilhamento_form():
    if st.session_state.phase != "tema_compartilhamento":
        return
    st.markdown('<div class="result-section-title">Quais temas você deseja compartilhar?</div>', unsafe_allow_html=True)
    temas = st.multiselect("Selecione um ou mais temas:", TEMAS_COMPARTILHAMENTO, key="input_tema_compartilhamento")
...
def render_final_result():
    if not st.session_state.quiz_completed:
        return
    st.markdown('<div id="card-resultado-avaliacao"></div><div class="result-section-title" style="display:block;">Resultado final da avaliação</div>', unsafe_allow_html=True)
    extra_info_html = ""
    if st.session_state.extra_question_answered and st.session_state.extra_question_answer:
        extra_info_html += f'<div class="final-result-info-item final-result-info-item-full"><span class="final-result-info-label">Interesse em compartilhar conhecimento?</span><span class="final-result-info-value">{st.session_state.extra_question_answer}</span></div>'
    if st.session_state.extra_question_answer == "Sim" and st.session_state.extra_question_topic:
        extra_info_html += f'<div class="final-result-info-item final-result-info-item-full"><span class="final-result-info-label">Tema que deseja abordar</span><span class="final-result-info-value">{st.session_state.extra_question_topic}</span></div>'
    level_class = get_level_class(st.session_state.level)
    st.markdown(f"""<div class="final-result-card">
<div class="final-result-top">
  <div><div class="final-result-name">{st.session_state.name}</div><div class="final-result-subtitle">Resultado do diagnóstico de conhecimento em IA</div></div>
...
<div class="final-result-info-grid">
  ...
  <div class="final-result-info-item"><span class="final-result-info-label">Versão</span><span class="final-result-info-value">{APP_VERSION}</span></div>
  {extra_info_html}
</div></div>""", unsafe_allow_html=True)
    with st.container():
        if st.button("🔄 Reiniciar teste", use_container_width=True, key="btn_reiniciar_dentro_card"):
            reiniciar_teste()
    st.markdown('<div class="result-section-title">Revisão das perguntas</div>', unsafe_allow_html=True)
    for item in st.session_state.results:
        status_class = "result-ok" if item["correct"] else "result-error"
        status_text = "Correta" if item["correct"] else "Incorreta"
        badge = '<span class="question-type-badge mc-badge">Múltipla Escolha</span>' if item.get("type") == "multiple_choice" else '<span class="question-type-badge open-badge">Pergunta Aberta</span>'
        st.markdown(f"""<div class="question-card">
<div style="display:flex;gap:8px;align-items:center;margin-bottom:14px;"><div class="question-status {status_class}">{status_text}</div>{badge}</div>
...
<div class="question-block"><div class="question-label">Resposta correta</div><div class="question-text">{item['reference']}</div></div>
...
def render_chat_actions():
    if not st.session_state.quiz_completed:
        return
    st.markdown('<div class="chat-actions-wrap"><a class="btn-ver-resultado-fixo" href="#card-resultado-avaliacao">📊 Ver resultado da avaliação</a></div>', unsafe_allow_html=True)
    if st.session_state.phase == "chat":
        st.markdown('<div class="chat-location-card">🤖 <strong>Chat para aperfeiçoamento</strong><br>O chat fica logo abaixo. Use o campo de mensagem para tirar dúvidas sobre inteligência artificial e consultar conteúdos.</div>', unsafe_allow_html=True)
...
A <strong>IA For Health</strong> apoia o desenvolvimento dos colaboradores da Santa Casa BH no tema de Inteligência Artificial.</div>
<div class="home-card">💡 <strong>Como funciona</strong>
<div class="como-funciona-grid compacto">
<div class="como-item"><strong>1. Identificação inicial</strong><br>Dados básicos para contextualizar sua experiência.<br><br>
<strong>2. Avaliação</strong><br>4 perguntas de múltipla escolha + 1 pergunta aberta.<br><br>
<strong>3. Análise</strong><br>Identificação do nível com base nas respostas.</div>
<div class="como-item"><strong>4. Aprendizado</strong><br>Chat para dúvidas, conceitos e exemplos práticos.<br>
&nbsp;&nbsp;- Tirar dúvidas<br>&nbsp;&nbsp;- Explorar conceitos<br>&nbsp;&nbsp;- Ver exemplos aplicados à saúde<br>&nbsp;&nbsp;- Acessar conteúdos internos</div>
...
def render_explicacao_nivel_final():
    ...
    cards = {
        "Básico": "🔴 <strong>Nível 1: Básico</strong> — Consumidor consciente. Usa IA para tarefas simples como resumos e e-mails.",
        "Intermediário": "🟡 <strong>Nível 2: Intermediário</strong> — Otimizador de processos. Domina engenharia de prompt e integra IA em fluxos de trabalho.",
        "Avançado": "🟢 <strong>Nível 3: Avançado</strong> — Arquiteto de soluções. Conecta tecnologias para criar automações e análises preditivas.",
    }
    card = cards.get(nivel, cards["Básico"])
    st.markdown(f'<div class="next-step-card">{card}</div><div class="next-step-card"><strong>Resumindo</strong><br><strong>Básico:</strong> Usa IA para tarefas simples.<br><strong>Intermediário:</strong> Domina técnica para resultados complexos.<br><strong>Avançado:</strong> Cria processos e automatiza de ponta a ponta.</div>', unsafe_allow_html=True)
...
def render_resumo_resultado():
    ...
    st.markdown(f"""<div class="final-result-card">
<div class="final-result-top">
  <div><div class="final-result-name">{st.session_state.name}</div><div class="final-result-subtitle">Resumo da sua avaliação</div></div>
  <div class="final-result-score-box"><div class="final-result-score-label">Pontuação</div><div class="final-result-score-value">{st.session_state.score}/5</div></div>
</div>
<div class="final-result-level-row">
...
<div class="final-result-info-grid">
  <div class="final-result-info-item"><span class="final-result-info-label">Nome</span><span class="final-result-info-value">{st.session_state.name}</span></div>
  <div class="final-result-info-item"><span class="final-result-info-label">Pontuação</span><span class="final-result-info-value">{st.session_state.score}/5</span></div>
  <div class="final-result-info-item"><span class="final-result-info-label">Nível</span><span class="final-result-info-value">{st.session_state.level}</span></div>
</div></div>""", unsafe_allow_html=True)
...
def render_chat_messages():
    ...
    # ── ÂNCORA DE FIM: altura 4px para ser detectável pelo scrollIntoView ──
...
def main():
    ...
    .final-result-level-row{display:flex;align-items:center;justify-content:flex-start;gap:8px;flex-wrap:wrap;margin-bottom:14px;}
    .final-result-level-label{font-size:.95rem;color:#334155;font-weight:700;}
    .final-result-level{display:inline-flex;align-items:center;padding:6px 14px;border-radius:999px;font-size:.84rem;font-weight:800;line-height:1;}
    .level-basico{background:#fff7ed;color:#c2410c;border:1px solid #fdba74;}
    .level-intermediario{background:#f3e8ff;color:#6300AB;border:1px solid #d8b4fe;}
    .level-avancado{background:#ecfdf3;color:#166534;border:1px solid #86efac;}
    ...
    /* ── Next step ── */
    .next-step-card{background:#faf5ff;border:1px solid #e9d5ff;color:#3b0764;border-radius:18px;padding:18px 20px;margin-top:8px;margin-bottom:10px;font-size:1rem;line-height:1.5;box-shadow:0 4px 16px rgba(99,0,171,.08);}
    ...
    /* ── Status bar ── */
    .status-bar{display:grid;grid-template-columns:repeat(5,1fr);gap:0;border:1px solid #e2e8f0;border-radius:12px;background:#fff;overflow:hidden;margin-bottom:4px;}
    ...
    @media(max-width:900px){
        ...
        .status-item:nth-child(4),.status-item:nth-child(5){border-top:1px solid #e2e8f0;}
    }
    ...
    [data-testid="stMainBlockContainer"],.block-container{
        ...
        /* CORRIGIDO: safe-area para iOS + espaço para o input fixo */
    ...
    /* ── Âncora de fim — detectável pelo scrollIntoView ── */
    #fim-da-pagina{
        height:4px;
        width:100%;
        display:block;
    ...
    /* ── iOS: evita elastic overscroll que bloqueia scroll programático ── */
    html,body{
    ...
    @media(max-width:768px){
        ...
        .final-result-card,.next-step-card,.chat-location-card,.question-card,.home-card{max-width:100%!important;overflow-wrap:break-word!important;word-break:break-word!important;}
        ...
        if not is_mc_waiting:
            ...
            if user_text:
                st.markdown(
                    ...
                    '<div class="chat-text">⏳ Processando...</div>'
                    '</div></div></div>',
                    unsafe_allow_html=True
                )
                try:
    ...
    run_focus_script()
    # SCROLL: sempre o ÚLTIMO elemento renderizado

...
def connect_google_sheet():
    ...
    sheet = client.open("Resultados_IA").worksheet("Resultados")
...
def save_result_to_file():
    ...
    if st.session_state.level in ["Intermediário", "Avançado"]:
        linhas.append(
            f"Deseja compartilhar conhecimentos: {st.session_state.extra_question_answer}"
    )

    if st.session_state.extra_question_answer == "Sim":
        linhas.append(
            f"Tema: {st.session_state.extra_question_topic}"
    )

    linhas.append(
        f"Data Hora Final: {format_datetime(st.session_state.test_end_time)}"
    )
